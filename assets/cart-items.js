...
if (!customElements.get('cart-items')) {
           ...
           if(cart.items.length == 1){
             cart.items.forEach(item => {
               if(item.handle == 'handling-fee'){
                    ...
                    }).then(response => response.json()).then(cartRes => {
                        fetch(`${window.Shopify.routes.root}cart`).then(response => response.text()) .then(data => {
                          const html = new DOMParser().parseFromString(data, 'text/html');
                          var drawer_data = html.querySelector('.cc-main-cart').innerHTML;  
                          document.querySelector('.cc-main-cart').innerHTML = drawer_data; 
  ...
  function addHandlingFee() {
    let cartItems = document.querySelector('cart-items');
    fetch('/cart.js').then(response => response.json()).then(cart => {
        let handlingFeeId = document.getElementById("handling-fee-id")?.value;
        if (!handlingFeeId) return;
        let hasFee = cart.items.some(item => item.id == handlingFeeId);
        if (!hasFee) {
            

            function finalcart(handlingFeeId) {
                fetch('/cart/add.js', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({id: handlingFeeId,quantity: 1})
                }).then(response => {
                  if (!response.ok) {throw new Error('Failed to add handling fee');} return response.json();
                }).then((res) => {
                        fetch(`${window.Shopify.routes.root}cart`).then(response => response.text()) .then(data => {
                          const html = new DOMParser().parseFromString(data, 'text/html');
                          var drawer_data = html.querySelector('.cart--with-items').innerHTML;
                          document.querySelector('.cart--with-items').innerHTML = drawer_data;
                        }).catch();
                }).catch(error => {});
            }
          
            let updates = {};
            updates[handlingFeeId] = 0;
            fetch(window.Shopify.routes.root + 'cart/update.js', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
       ...
       }).catch(error => console.error("Handling Fee Error:", error));
  }
  document.addEventListener("DOMContentLoaded", addHandlingFee);
