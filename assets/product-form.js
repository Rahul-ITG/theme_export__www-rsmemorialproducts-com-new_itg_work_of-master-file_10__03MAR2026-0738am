if (!customElements.get('product-form')) {
  class ProductForm extends HTMLElement {
    ...
    async handleSubmit(evt) {
      ...
      // ----------------------------- Add to cart (AdditionalProduct) --------------------------
      let AdditionalProduct = document.querySelectorAll('.SwymGrPositionSticky .Additional-fields-wrapper.additional-product');
      if(AdditionalProduct.length > 0){
        AdditionalProduct.forEach((item,index) => {
            let textarea = item.querySelector('textarea');
            if (textarea && textarea.value.length > 0) {
               let variantId = item.getAttribute('data-var-id');
               formData.set(`items[${index}][id]`,variantId);  
            } 
            else {
              let fileInput = item.querySelector('[type="file"]');
...
function updateAddToCartButtonText() {
    var button_new = document.querySelector('[data-add-to-cart-text][name="add"]');
    if(button_new){
      var uploadPreviewImage = document.getElementById('UploadPreviewImage');
      var unavailableText = button_new.textContent;
      if (button_new && uploadPreviewImage) {
          // Update text in #UploadPreviewImage to match button text
          uploadPreviewImage.textContent = button_new.getAttribute('data-add-to-cart-text');
          // If button is disabled, disable the #UploadPreviewImage span too
          if (button_new.disabled) {
              uploadPreviewImage.setAttribute('aria-disabled', 'true');
              uploadPreviewImage.classList.add('disabled'); // Optionally add a class to style it
              uploadPreviewImage.textContent = unavailableText;
          } else {
              uploadPreviewImage.removeAttribute('aria-disabled');
              uploadPreviewImage.classList.remove('disabled'); // Remove the class if it's enabled
              uploadPreviewImage.textContent = unavailableText;
          }
      }
    }
...
// Observe changes in the button's text content and disabled attribute
var button_new2 = document.querySelector('[data-add-to-cart-text][name="add"]');
if (button_new2) {
    var observer = new MutationObserver(updateAddToCartButtonText);
    // Observe changes in the text and disabled state
    observer.observe(button_new2, { 
        attributes: true, 
        childList: true, 
        subtree: true 
    });
...
// Initial update in case the button state is already set
